...
const OrderSummary = ({updatedOrder}) => {
  ...
  // console.log(updatedOrder?.reduce((acc, item) => acc + item.orderItems[0].product.price,0))

  return (
    <div className="w-full p-5 rounded-lg shadow-md border-[1px] border-gray-300">
          ...
          <div className="flex sm:hidden items-center justify-between mb-4">
            <div className="relative w-3/4 px-2">
              ...
              <input
                ...
                // onKeyPress={(e) => {
                //   if (e.key === 'Enter') {
                //     handleApplyPromoCode(e.target.value);
                //     e.target.value = '';
                //   }
                // }}
              />
            </div>
            <button className="bg-black text-white px-4 py-2 rounded-3xl hover:border-gray-300 hover:bg-transparent hover:text-inherit hover:border-[2px]">
              Apply
            </button>
          </div>
